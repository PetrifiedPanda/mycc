...
static struct token create_value(struct value val, size_t line, size_t index) {
    ...
    if (val.type == VALUE_FLOAT || val.type == VALUE_DOUBLE || val.type == VALUE_LDOUBLE) {
        type = F_CONSTANT;
    }
    return (struct token) {
        .type = type,
...
TEST(file) {
    ...
    struct token expected[] = {
        ...
        create_value(create_float_value(VALUE_DOUBLE, 1e10), 66, 24),
        ...
        create_value(create_int_value(VALUE_INT, 10), 115, 51),
        create(COMMA, NULL, 115, 53),
        create(DOUBLE, NULL, 115, 55),
        create(COLON, NULL, 115, 61),
        create_value(create_float_value(VALUE_DOUBLE, 12.0), 115, 63),
