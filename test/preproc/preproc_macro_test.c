...
TEST(func_like) {
    ...
    test_preproc_macro(&macro1, 3, 14, "int n = FUNC_LIKE_MACRO(2 * 2, x - 5 + 2) + 1;", "int n = 2 * 2 + x - 5 + 2 * 3 - x - 5 + 2 + 1;");
    test_preproc_macro(&macro1, 3, 7, "char c = FUNC_LIKE_MACRO(, 10);", "char c = + 10 * 3 - 10;");
    test_preproc_macro(&macro1, 2, 6, "f = FUNC_LIKE_MACRO(f,);", "f = f + * 3 -;");
    ...
    test_preproc_macro(&macro2, 0, 13, "OTHER_FUNC_LIKE(var, 1, 2, 3, 4, 5) = 69;", "var = 69;");
    ...
    test_preproc_macro(&macro3, 4, 6, "const float stuff = YET_ANOTHER_FUNC_LIKE();", "const float stuff = 1 + 1;");
}
