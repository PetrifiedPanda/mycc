...
Token* tokenize(const char* str) {
    ...
    if (!res.tokens) {
        set_error(ALLOC_FAIL, "Failed to allocate token array");
        goto fail;
    }
        ...
        if (type != INVALID && is_valid_singlec_token(type, s.prev, s.prev_prev)) {
            if (type == DIV) {
                if (handle_comments(&s)) {
                    continue;
                } else if (get_last_error() != NO_ERROR) {
                    goto fail;
                }
            }
            if (s.it[1] != '\0') {
    ...
    if (!new_tokens) {
        set_error(ALLOC_FAIL, "Failed to resize token array");
        goto fail;
    }
...
static bool realloc_tokens_if_needed(size_t token_idx, TokenArr* res) {
    if (token_idx == res->len) {
        ...
        if (!new_tokens) {
            set_error(ALLOC_FAIL, "Failed to reallocate Token Array");
            return false;
        }
        res->tokens = new_tokens;
    }
...
static void unterminated_literal_err(char terminator, size_t line, size_t start_idx) {
    ...
    set_error(TOKENIZER, "%s literal at line %ul char %ul not properly terminated", literal_type_str, line, start_idx);
}
...
static bool handle_comments(TokenizerState* s) {
    if (s->it[1] == '*') {
        ...
        if (*s->it == '\0') {
            set_error(TOKENIZER, "Comment was not properly terminated");
            return false;
        } else {
            assert(*s->it == '*');
            assert(s->it[1] == '/');
...
static bool handle_character_literal(TokenizerState* s, TokenArr* res, size_t* token_idx) {
    ...
    if (*s->it == '\0') {
        ...
        if (!dyn_buf) {
            set_error(ALLOC_FAIL, "Could not allocate buffer for long literal");
            return false;
        }
        ...
        while (*s->it != '\0' && *s->it != terminator) {
            if (buf_idx == dyn_buf_lim) {
                ...
                if (!new_buf) {
                    set_error(ALLOC_FAIL, "Could not enlarge buffer for long string literal");
                    free(dyn_buf);
                    return false;
                } else {
                    dyn_buf = new_buf;
        ...
        if (!new_buf) {
            set_error(ALLOC_FAIL, "Could not resize buffer for long string literal");
            free(dyn_buf);
            return false;
        }
        dyn_buf = new_buf;
        ...
        if (type == INVALID || !add_token_move(token_idx, res, type, dyn_buf, s->line, start_idx)) {
            if (type == INVALID) {
                set_error(TOKENIZER, "Terminated literal is of unknown type");
            }
            free(dyn_buf);
            return false;
        }
...
static bool handle_other(TokenizerState* s, TokenArr* res, size_t* token_idx) {
    ...
    if (type != INVALID && token_is_over(s)) {
        ...
        if (is_hex_const(spell_buf, buf_idx) || is_oct_const(spell_buf, buf_idx) || is_dec_const(spell_buf, buf_idx) || is_float_const(spell_buf, buf_idx)) {
            ...
            set_error(TOKENIZER, "Invalid identifier");
            return false;
        }
        ...
        set_error(TOKENIZER, "Identifier too long");
