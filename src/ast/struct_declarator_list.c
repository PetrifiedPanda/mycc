...
struct struct_declarator_list parse_struct_declarator_list(struct parser_state* s) {
    ...
    if (!parse_struct_declarator_inplace(s, &res.decls[0])) {
        free(res.decls);
        return (struct struct_declarator_list) {
            .len = 0,
            .decls = NULL
        };
    }
    ...
    while (s->it->type == COMMA) {
        ...
        if (res.len == alloc_len) {
            grow_alloc((void**)&res.decls, &alloc_len, sizeof(struct struct_declarator));
        }
