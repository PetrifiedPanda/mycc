...
int main(int argc, char** argv) {
    ...
    if (args.enable_new_parser) {
        if (args.action == ARG_ACTION_CONVERT_BIN_TO_TEXT) {
            for (uint32_t i = 0; i < args.num_files; ++i) {
                if (!convert_bin_to_text_2(&args, args.files[i])) {
                    goto fail;
                }
            }
        } else {
            for (uint32_t i = 0; i < args.num_files; ++i) {
                if (!output_ast_2(&args, &type_info, args.files[i])) {
                    goto fail;
                }
            }
        }
...
static bool convert_bin_to_text_2(const CmdArgs* args, CStr filename) {
    ...
    DeserializeASTRes_2 res = deserialize_ast_2(in_file);
    if (res.ast.len == 0) {
        File_printf(mycc_stderr,
                    "Failed to read ast from file {Str}\n",
                    filename);
    ...
    if (!dump_ast_2(&res.ast, &res.file_info, out_file)) {
        File_printf(mycc_stderr,
                    "Failed to write ast to textfile {Str}\n",
                    out_filename);
        goto fail_with_out_file_open;
