...
static void insert_test_helper(Str* str, char* expected_buf, size_t initial_len) {
...
TEST(push_back_to_empty_with_cap) {
    ...
    Str str3 = create_empty_str_with_cap(STR3_CAP);
    ASSERT_SIZE_T(str_cap(&str3), STR3_CAP);
    memset(expected_buf, 0, sizeof expected_buf);
    insert_test_helper(&str3, expected_buf, 0);

    Str str4 = create_empty_str_with_cap(0);
...
#define PUSH_BACK_TEST_HELPER(str_lit)                                         \
    do {                                                                       \
        ...
        Str str = create_str(RAW_STR_STRLEN, raw_str);                  \
...
#define SMALL_B "cd"
    ...
    Str small_str = str_concat(SMALL_A_LEN,
                                      SMALL_A,
                                      SMALL_B_LEN,
                                      SMALL_B);
...
#define LARGE_B "that will be allocated in the dynamic buffer"
    ...
    Str large_str = str_concat(LARGE_A_LEN,
                                      LARGE_A,
                                      LARGE_B_LEN,
                                      LARGE_B);
...
TEST(str_shrink_to_fit) {
    {
        ...
        Str str = create_empty_str_with_cap(STATIC_BUF_LEN);
...

    {
        ...
        Str str = create_empty_str_with_cap(INIT_CAP);
...
#define FULL_STR STR_START STR_END
    Str str = create_str(ARR_LEN(FULL_STR) - 1, FULL_STR);
...
TEST(str_pop_back) {
    ...
    Str str = create_str(EX_STRLEN, ex_str);
