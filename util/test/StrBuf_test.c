...
static void insert_test_helper(StrBuf* str,
    ...
    for (size_t i = 0; i < INSERTION_NUM; ++i) {
        const size_t expected_len = initial_len + i;
        ASSERT_STR(StrBuf_as_str(str), ((Str){expected_len, expected_buf}));
        ASSERT_SIZE_T(StrBuf_len(str), expected_len);
...
TEST(push_back_to_empty_with_cap) {
    ...
    ASSERT_SIZE_T(StrBuf_cap(&str3), STR3_CAP);
    memset(expected_buf, 0, sizeof expected_buf);
    insert_test_helper(&str3, expected_buf, 0);

    StrBuf str4 = StrBuf_create_empty_with_cap(0);
    ASSERT_SIZE_T(StrBuf_cap(&str4), STATIC_BUF_LEN);
    memset(expected_buf, 0, sizeof expected_buf);
...
TEST(reserve) {
    ...
    ASSERT_SIZE_T(StrBuf_cap(&str), STATIC_BUF_SIZE - 1);
    ...
    ASSERT_STR(StrBuf_as_str(&str), ex_str);
    ASSERT_SIZE_T(StrBuf_cap(&str), TO_RESERVE_2);
    ...
    ASSERT_SIZE_T(StrBuf_cap(&str), TO_RESERVE_3);
...
#define CONCAT_TEST_HELPER(lit1, lit2, ex)                                     \
    do {                                                                       \
        ...
        ASSERT_SIZE_T(StrBuf_len(&res), (size_t)expected.len);                 \
...
TEST(shrink_to_fit) {
    {
        ...
        ASSERT_STR(StrBuf_as_str(&str), ex_str);
        ASSERT_SIZE_T(StrBuf_cap(&str), STATIC_BUF_LEN);
        StrBuf_shrink_to_fit(&str);
        ASSERT_SIZE_T(StrBuf_cap(&str), STATIC_BUF_LEN - 1);
        ASSERT_STR(StrBuf_as_str(&str), ex_str);
...

    {
        ...
        const size_t init_cap = c_str.len * 3;
        StrBuf str = StrBuf_create_empty_with_cap(init_cap);
        ASSERT_SIZE_T(StrBuf_cap(&str), init_cap);
        StrBuf_append(&str, c_str);
        ASSERT_STR(StrBuf_as_str(&str), c_str);
        ASSERT_SIZE_T(StrBuf_len(&str), c_str.len);

        StrBuf_shrink_to_fit(&str);
        ASSERT_SIZE_T(StrBuf_cap(&str), c_str.len);
        ASSERT_STR(StrBuf_as_str(&str), c_str);
        StrBuf_free(&str);
    }
}
...
TEST(pop_back) {
    ...
    while (curr_len != 0) {
        ...
        ex_str.len -= 1;
        ASSERT_STR(StrBuf_as_str(&str), ex_str);
        ASSERT_SIZE_T(StrBuf_len(&str), curr_len);
    }
    StrBuf_free(&str);
}
