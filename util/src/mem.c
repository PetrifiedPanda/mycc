...
static size_t find_alloc_idx(void* alloc) {
    assert(alloc != NULL);
    if (g_alloc_entries.len == 0) {
        return 0;
    }
    size_t left = 0;
    size_t right = g_alloc_entries.len == 0 ? 0 : g_alloc_entries.len - 1;
    while (left <= right) {
        const size_t middle = floor((left + right) / 2.0);
        if (g_alloc_entries.data[middle].alloc < alloc) {
            left = middle + 1;
        } else if (g_alloc_entries.data[middle].alloc > alloc) {
            right = middle - 1;
        } else {
            return middle;
        }
...
static void memdebug_cleanup(void) {
    ...
    size_t alloced = 0;
    size_t freed = 0;
    for (size_t i = 0; i < g_alloc_entries.len; ++i) {
        struct alloc_entry* curr = &g_alloc_entries.data[i];
        alloced += curr->bytes;
        if (curr->freed) {
            freed += curr->bytes;
        } else {
...
static void insert_alloc(void* alloc,
    ...
    if (idx >= g_alloc_entries.len
        || g_alloc_entries.data[idx].alloc != alloc) {
        if (g_alloc_entries.len == g_alloc_entries.cap) {
            mycc_grow_alloc((void**)&g_alloc_entries.data,
                            &g_alloc_entries.cap,
                            sizeof *g_alloc_entries.data);
        }

        for (size_t i = g_alloc_entries.len; i > idx; --i) {
            assert(g_alloc_entries.data[i - 1].alloc > alloc);
            g_alloc_entries.data[i] = g_alloc_entries.data[i - 1];
        }
        ++g_alloc_entries.len;
    }

    g_alloc_entries.data[idx] = create_alloc_entry(alloc, bytes, file, line);
...
static void set_freed(size_t alloc_idx,
                      ...
                      const char* file,
                      size_t line) {
    struct alloc_entry* curr = &g_alloc_entries.data[alloc_idx];
    assert(!curr->freed);
    curr->freed = true;
...
static void check_if_freed(size_t alloc_idx) {
    const struct alloc_entry* entry = &g_alloc_entries.data[alloc_idx];
    if (entry->freed) {
...
void* mycc_memdebug_realloc_wrapper(void* alloc,
    ...
    if (alloc == NULL) {
        ...
        assert(g_alloc_entries.data[alloc_idx].alloc == alloc);
